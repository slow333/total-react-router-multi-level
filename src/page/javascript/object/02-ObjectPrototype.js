export default function ObjectPrototype() {
   return(<main>
   ...
   <code>{`Person.prototye; Person 객체와 상속되는 prototype 객체를 볼수 있음
      Object.prototype; 해당 객체의 상속 받는 prototype 객체를 볼수 있음
      String.prototype; Date.prototype; Number.prototype; Array.prototype;`}</code>
      </section>
      <section>
         <h3>create()</h3>
   <code>{`var person2 = Object.create(person1);
      person2.__proto__; <= person1을 프로토타입 객체로 삼음`}</code>
         <h3>constructor, 생성자 속성</h3>
         <code>{`person1.constructor; person2.constructor;`}</code>
   ...
   <code>{`function Person (first, last, age,...) {
      ...
      <code>{`Person.prototype.fullName = this.name.first + " " + this.name.last;`}</code>
      ...
      <p><b>사실 일반적인 방식으로는 속성은 생성자에서, 메소드는 프로토타입에서 정의합니다.
         <br/>생성자에는 속성에 대한 정의만 있으며 메소드는 별도의 블럭으로 구분할 수 있으니 코드를 읽기가 훨씬 쉬워집니다. 아래처럼요:</b></p>
   <code>{`// 생성자에서 속성 정의
