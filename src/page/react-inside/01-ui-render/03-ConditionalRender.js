  ...
  return (
     <main>
        ...
        <section>
           ...
           <p>아직 다 활용하지 않았을 수도 있지만 React에는 렌더링하면서 읽을 수 있는 세 가지 종류의 입력 요소가 있습니다. props, state, 그리고 context. 이러한 입력 요소는 항상
              읽기전용으로 취급해야 합니다.<br/>
           ...
           <ul>
              <li>컴포넌트는 다른 환경에서도 실행될 수 있습니다- 예를 들면 서버에서 말이죠! 동일한 입력에 대해 동일한 결과를 반환하기 때문에 하나의 컴포넌트는 많은 사용자 요청을 처리할 수
                 있습니다.
              </li>
              <li>입력이 변경되지 않은 컴포넌트 렌더링을 건너뛰어 성능을 향상시킬 수 있습니다. 순수 함수는 항상 동일한 결과를 반환하므로 캐시하기에 안전합니다.</li>
              <li>깊은 컴포넌트 트리를 렌더링하는 도중에 일부 데이터가 변경되는 경우 React는 오래된 렌더링을 완료하는 데 시간을 낭비하지 않고 렌더링을 다시 시작할 수 있습니다. 순수함은 언제든지
                 연산을 중단하는 것을 안전하게 합니다.
              </li>
           </ul>
        </section>
        <section>
           <h2>side effect, outside effect</h2>
           <p>함수형 프로그래밍은 순수성에 크게 의존하지만, 언젠가는, 어딘가에서, 무언가가 바뀌어야 합니다. 그것이 프로그래밍의 요점입니다! 이러한 변화들(화면을 업데이트하고, 애니메이션을 시작하고,
              데이터를 변경하는 것)을 SIDE EFFECT사이드 이펙트라고 합니다. <b>렌더링중에 발생하는 것이 아니라 “사이드에서,” 발생하는 현상입니다.</b></p>
           <p>React에선, 사이드 이펙트는 보통 이벤트 핸들러에 포함됩니다. 이벤트 핸들러는 React가 일부 작업을 수행할 때 반응하는 기능입니다-예를 들면 버튼을 클릭할 때처럼 말이죠. 이벤트
              핸들러가 컴포넌트 내부에 정의되었다 하더라도 렌더링 중에는 실행되지 않습니다! 그래서 이벤트 핸들러는 순수할 필요가 없습니다.</p>
